...
def extract(dir_path, model_name_or_path, out_file, batch_size=1):
    ...
    model = SpeechT5ForSpeechToSpeech.from_pretrained(model_name_or_path
                                                     ).to(device).eval()
    ...
    with h5py.File(out_file, "w") as h5f:
        for i in tqdm(range(0, len(wav_set), batch_size), desc="Extract"):
            ...
            for j, s in enumerate(batch_samples):
                ...
                if attn_mask is not None:
                    valid_len = int(
                        attn_mask[j].sum().item()
                    )  # number of valid frames
                    vec = enc[j, :valid_len, :].numpy().astype(
                        np.float32
                    )  # shape (T_i, D)
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dir", required=True, help="folder containing 16 kHz wavs"
    )
