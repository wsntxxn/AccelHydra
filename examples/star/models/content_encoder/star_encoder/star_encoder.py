...
class QformerBridgeNet(torch.nn.Module):
    def __init__(
        ...
        hiddin_size: int = 1024,
        speech_width: int = 1024,
        ...
        self.audio_Qformer, self.audio_query_tokens, encoder_config = self.init_Qformer(
            num_query_token=num_query_token, speech_width=speech_width
        )
        ...
        self.hiddin_projection = torch.nn.Linear(
            encoder_config.hidden_size, hiddin_size
        )
        ...
        if load_from_pretrained:
            ...
            del_state_dict = {
                k: v
                for k, v in state_dict.items() if k not in del_key
            }
